...
parser.add_argument('--input_params', nargs="*", action='append',
  help="values for input parameters.")

#render_quality_prob1
#render_quality_prob2
#render_quality_prob3
#image_size_1
#image_size_2
#image_size_3
#bounces_1
...
def main(args):
  args.input_params = [float(i) for i in args.input_params[0]]
  num_digits = 6
  ...
  for i in range(args.num_images):
    ...
    probabilities_quality_choices=softmax(np.asarray([args.input_params[0], args.input_params[1], args.input_params[2]]))
    ...
    ###################################################################
    #######################   IMAGE_SIZE  #############################
    imagesize_choices=[[32,32],[320,240],[640,640]]
    probabilities_size_choices=softmax(np.asarray([args.input_params[3], args.input_params[4], args.input_params[5]]))
    size_n = np.random.multinomial(1, probabilities_size_choices)
    size_n = imagesize_choices[size_n.tolist().index(1)]
    args.width, args.height=size_n[0], size_n[1]
    ###################################################################
    ##########################   BOUNCES  #############################
    bounces_choices=[8,128]
    probabilities_bounces_choices=softmax(np.asarray([args.input_params[6], args.input_params[7]]))
    bounces_n = np.random.multinomial(1, probabilities_bounces_choices)
    bounces_n = bounces_choices[bounces_n.tolist().index(1)]
    args.render_max_bounces=bounces_n
    ###################################################################
    ####################   LIGHTS STRENGTH  ###########################
    light_strength_1=np.random.normal(args.input_params[8], 0.05, 1)
    light_strength_2=np.random.normal(args.input_params[9], 0.05, 1)
    ###################################################################
    ##########################   LOCATION  ############################
    args.loc=np.random.normal(args.input_params[10], 0.05, 1)
    ###################################################################
    ####################   MATERIAL  ###########################
    material_mapping=[("Rubber", "rubber"), ("MyMetal", "metal")]
    probabilities_material_choices=softmax(np.asarray([args.input_params[11], args.input_params[12]]))
    materials_n = np.random.multinomial(1, probabilities_material_choices)
    mat_name, mat_name_out = material_mapping[materials_n.tolist().index(1)]
    ###################################################################
...
def render_scene(args,num_objects=5,output_index=0,output_split='none',output_image='render.png',output_scene='render_json',output_blendfile=None,camera_jitter=None,key_light_jitter=None,back_light_jitter=None,fill_light_jitter=None,objects_info=None, light_strength=None):
