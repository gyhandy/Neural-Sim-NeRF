...
parser.add_argument('--obj_name', default='Sphere',
	help="name of object")
parser.add_argument('--obj_name_out', default='cylinder',
...
parser.add_argument('--params_list', nargs="*", action='append',
	help="values for input parameters.")
...
def main(args):
	...
	for i in range(args.num_images):
		...
		print((args.params_list[0]))
		params_list=args.params_list[0]
		print(args.params_list)
		params_list_new=[]
		print(args.num_images)
		print(len(params_list))
		for in_par in range(int(len(params_list)/args.num_images)):
			list_a_new=[]
			for b_par in range(args.num_images):
				list_a_new.append(float(params_list[in_par*args.num_images+b_par]))
			print(list_a_new)
			params_list_new.append(list_a_new)
		params_list = params_list_new
		print(params_list)
		...
		quality_choices=[8,64,512]
		args.render_num_samples=quality_choices[int(params_list[0][i])]
		imagesize_choices=[[64,64],[128,128],[256,256]]
		# imagesize_choices=[[32,32], [64,64],[256,256]]
		args.width, args.height=imagesize_choices[int(params_list[1][i])][0], imagesize_choices[int(params_list[1][i])][1]
		bounces_choices=[8,128]
		args.render_max_bounces=bounces_choices[int(params_list[2][i])]
		light_strength_1=params_list[3][i]
		light_strength_2=params_list[4][i]
		########################################################################################
		###################                   OBJECT Info                    ###################
		...
		objects_info = []
		x_list=[]
		locs=random.sample([params_list[5][i], params_list[6][i], params_list[7][i]], num_objects )

		for i_ob in range(num_objects):
			...
			if obj_name == 'SmoothCube_v2':
				...
				color_name = "green"
				rgb = [29, 105, 20]
			elif obj_name == 'SmoothCylinder':
				obj_name_out = 'cylinder'
				cl_ob=2
				color_name = "yellow"
				rgb = [255, 238, 51]
			rgba = [float(c) / 255.0 for c in rgb] + [1.0]

			material_mapping=["Rubber", "MyMetal"]
			mat_name = material_mapping[int(params_list[8+cl_ob][i])]
			if mat_name == 'Rubber':
				mat_name_out = 'rubber'
			elif mat_name == 'MyMetal':
				mat_name_out = 'metal'
			# y = random.uniform(-2.8, 3.2)
			# x = random.uniform(-args.x_loc, args.x_loc)
			# x=args.x_loc
			x=np.random.normal(int(locs[i_ob]), 0.05, 1)
			print(x)
			to_avoid=0
			for x_old in x_list:
				if x_old - x <1:
			...
			# color_name, rgba = list(color_name_to_rgba.items())[cl_ob]
			...
			# material_mapping[materials_n.tolist().index(1)]
		...
		render_scene(args,
			...
			light_strength=[light_strength_1, light_strength_2],
		)
...
def render_scene(args,num_objects=5,output_index=0,output_split='none',output_image='render.png',output_label='label.png',output_scene='render_json',output_blendfile=None,camera_jitter=None,key_light_jitter=None,back_light_jitter=None,fill_light_jitter=None,objects_info=None, light_strength=None):
	...
	############################################################
	########### MODIFYING LIGHT STRENGTH #######################
	bpy.data.objects['Area'].data.node_tree.nodes['Emission'].inputs['Strength'].default_value = light_strength[0] # from [0, 100]
	bpy.data.objects['Lamp_Back'].data.node_tree.nodes['Emission'].inputs['Strength'].default_value = light_strength[1] # from [0, 100]

	# Now make some random objects
	objects, blender_objects = add_random_objects(scene_struct, num_objects, args, camera, objects_info)
	...
	for _id in box_dict:
		...
		if objects[_id]['shape'] == 'cube':
			objects[_id]['label'] = 0
		elif objects[_id]['shape'] == 'sphere':
			objects[_id]['label'] = 1
		elif objects[_id]['shape'] == 'cylinder':
			objects[_id]['label'] = 2
		print(_id)
		print(objects[_id]['shape'])
		# print(objects[_id]['points'])
		# print(len(objects[_id]['points']))
		for point in objects[_id]['points']:
			# print(point)
			# print(modif_point)
			label_image[args.height-int(clamp(point[1], 0.0, args.height-1)), int(clamp(point[0], 0.0, args.width-1))] = objects[_id]['label']+1
			# label_image[args.height-int(clamp(point[1], 0.0, args.height-1)), int(clamp(point[0], 0.0, args.width-1))] = 1
		print(objects[_id]['label']+1)
	...
	img1 = Image.fromarray(label_image, mode='L')
	img1.save(output_label)
	object_colors = render_shadeless(blender_objects, path=output_label)
...
def add_random_objects(scene_struct, num_objects, args, camera, objects_info):
	"""
	...
	for i in range(len(objects_info)):
		...
		utils.add_object(args.shape_dir, objects_info[i]['obj_name'], objects_info[i]['r'], (objects_info[i]['x'], objects_info[i]['y']), theta=objects_info[i]['theta'])
		obj = bpy.context.object
		blender_objects.append(obj)
		positions.append((objects_info[i]['x'], objects_info[i]['y'], objects_info[i]['r']))

		
		utils.add_material(objects_info[i]['mat_name'], Color=objects_info[i]['rgba'])

		# Record data about the object in the scene data structure
		pixel_coords = utils.get_camera_coords(camera, obj.location)
		print(objects_info[i]['x'], objects_info[i]['y'], obj.location, pixel_coords)
		objects.append({
			'shape': objects_info[i]['obj_name_out'],
			'size': objects_info[i]['size_name'],
			'material': objects_info[i]['mat_name_out'],
			'3d_coords': tuple(obj.location),
			'rotation': objects_info[i]['theta'],
			'pixel_coords': pixel_coords,
			'color': objects_info[i]['color_name'],
		})
	...
	# 	return add_random_objects(scene_struct, num_objects, args, camera, objects_info)
...
def render_shadeless(blender_objects, path='flat.png'):
	...
	for i, obj in enumerate(blender_objects):
		...
		if obj.data.name == 'Cube':
			mat.diffuse_color = cityscapes_map[1]
			object_colors.add((cityscapes_map[1][0], cityscapes_map[1][1], cityscapes_map[1][2]))
		elif obj.data.name == 'Icosphere':
			# print('sphere', cityscapes_map[2])
			mat.diffuse_color = cityscapes_map[2]
			object_colors.add((cityscapes_map[2][0], cityscapes_map[2][1], cityscapes_map[2][2]))
		elif obj.data.name == 'Cylinder.007':
			# print('cylinder')
			# print(cityscapes_map[3])
			mat.diffuse_color = cityscapes_map[3]
			object_colors.add((cityscapes_map[3][0], cityscapes_map[3][1], cityscapes_map[3][2]))
