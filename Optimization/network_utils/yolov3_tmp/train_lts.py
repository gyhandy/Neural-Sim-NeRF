...
def train_yolo_lts(opt):
    if opt.save_location is None:
        wdir = 'network_utils/yolov3_tmp/weights' + os.sep  # weights dir
        results_file = 'network_utils/yolov3_tmp/results.txt'
    else:
        wdir = opt.save_location + os.sep + 'weights' + os.sep # weights dir
        results_file = opt.save_location + os.sep + 'results_files' + os.sep + 'results_%s.txt' % opt.name
    last = wdir + 'last_%s.pt' % opt.name
    best = wdir + 'best_%s.pt' % opt.name
    ...
    if opt.train_txt is None:
        train_path = data_dict['train']
    else:
        train_path = opt.train_txt
    ...
    # Initialize model
    model = Darknet(cfg, arc=opt.arc, transfer=False, freeze_layers=12).to(device)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------
        ...
        if opt.cutoff >0 and epoch < 1:
            for name, p in model.named_parameters():
                if int(name.split('.')[1]) < opt.cutoff:  # if layer < 75
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            ...
            results, maps = yolo_test.yolo_test(cfg,
                                      data,
                                      batch_size=batch_size * 2,
        ...
        if fi > best_fitness and not opt.evolve:
            best_fitness = fi
    ...
    if len(n):
        ...
        os.rename(results_file, results_file.strip('.txt') + n + '.txt')
        ...
        n = train_path.split('../icml_experiments_data/')[1]
        n=n.replace('/', '')
        n=n.strip('.txt')
        n = '_' + n if not n.isnumeric() else n
        fresults, flast, fbest, fresultspng = 'results%s.txt' % n, 'last%s.pt' % n, 'best%s.pt' % n, 'results%s.png' % n
        os.rename('results.txt', fresults)
        os.rename('results.png', fresultspng) if os.path.exists('results.png') else None
        os.rename(wdir + 'last.pt', wdir + flast) if os.path.exists(wdir + 'last.pt') else None
        os.rename(wdir + 'best.pt', wdir + fbest) if os.path.exists(wdir + 'best.pt') else None
    ...
    return results, fi, best_fitness, dataloader, testloader, model, dataset, device
